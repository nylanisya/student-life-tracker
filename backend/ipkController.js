...
const getIpkHistory = async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT * FROM ipk_history ORDER BY semester ASC"
    );
...
const getCurrentIpk = async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT * FROM ipk_history ORDER BY semester DESC LIMIT 1"
    );
    if (rows.length === 0) {
...
const addIpk = async (req, res) => {
  ...
  if (!semester || ipk === undefined || ipk === null) {
    return res
      .status(400)
      .json({ success: false, message: "Semester dan IPK wajib diisi" });
  }
  ...
  try {
    ...
    const [newRecord] = await db.query(
      "SELECT * FROM ipk_history WHERE id = ?",
      [result.insertId]
    );
...
const updateIpk = async (req, res) => {
  ...
  try {
    ...
    const [updated] = await db.query("SELECT * FROM ipk_history WHERE id = ?", [
      id,
    ]);
    if (updated.length === 0) {
      return res
...
const deleteIpk = async (req, res) => {
  ...
  try {
    const [result] = await db.query("DELETE FROM ipk_history WHERE id = ?", [
      id,
    ]);
